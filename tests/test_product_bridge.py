...
class ProductBridgeApiTests(unittest.TestCase):
    ...
    def test_text_and_voice_use_same_execution_path_and_conversation(self):
        for mode in ("text", "voice"):
            ...
            self.assertEqual(payload["conversation_id"], "conversation-42")
            self.assertEqual(payload["status"], "complete")
        for _, kwargs in self.agent.calls:
            self.assertEqual(kwargs["user_id"], "product:abc123")
            self.assertEqual(kwargs["conversation_id"], "conversation-42")
    ...
    def test_bridge_requires_auth_and_user_identity(self):
        response = self.client.post("/v1/product/execute", json={"instruction": "Research this"})
