...
class JobQueue:
    ...
    def enqueue(self,kind:str,payload:dict[str,Any],priority:int=100,max_attempts:int=3,run_after:str|None=None)->str:
        ...
        with connect(self.path) as c:c.execute('INSERT INTO jobs(id,kind,payload,status,priority,max_attempts,run_after,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)',(i,kind,json.dumps(payload),'queued',priority,max_attempts,run_after or t,t,t))
        return i
    def claim(self)->dict[str,Any]|None:
        with connect(self.path,timeout=10,isolation_level='IMMEDIATE') as c:
    ...
    def complete(self,i:str,result:Any)->None:
        with connect(self.path) as c:c.execute("UPDATE jobs SET status='complete',result=?,updated_at=? WHERE id=?",(json.dumps(result,default=str),now(),i))
