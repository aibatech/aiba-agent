...
class Scheduler:
    ...
    def _init(self):
        with sqlite3.connect(self.path) as c:c.execute('''CREATE TABLE IF NOT EXISTS schedules(id TEXT PRIMARY KEY,name TEXT NOT NULL,kind TEXT NOT NULL,payload TEXT NOT NULL,interval_seconds INTEGER NOT NULL,next_run TEXT NOT NULL,enabled INTEGER NOT NULL DEFAULT 1)''')
    def add_interval(self,name:str,kind:str,payload:dict,seconds:int)->str:
        ...
        with sqlite3.connect(self.path) as c:c.execute('INSERT INTO schedules VALUES(?,?,?,?,?,?,1)',(i,name,kind,json.dumps(payload),seconds,nxt))
        return i
    def tick(self)->int:
        now=now_dt();count=0
